...
`/api/overview` may expose compact inventory counts for portals and status cards, but the full per-skill `items[]` list belongs in `/api/status` and the secondary `Skill 清单` view.
