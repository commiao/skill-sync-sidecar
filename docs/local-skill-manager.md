...
After a first-screen action runs, the first card should retain the recent non-transient result for a short time: success, no write, rejected because the source changed, waiting for device reporting, or failure. Do not retain `正在...` progress states after rerenders; users need the outcome and next safe step, not stale progress.
Ordinary OpenClaw `source_changed` review should be labeled as `可稍后处理` on the first screen. It is a normal editing/sync review state, not a service fault and not a blocker for current-client skill management. Conflict, delete review, fetch failure, stale device state, and write failures are the states that should interrupt the user as higher-risk work.
When ordinary OpenClaw review is the only first-screen item, use a neutral/deferrable visual treatment and label the count as `普通待审`, not generic `待办`. This keeps the first card from looking like a blocked yellow error state.
The secondary confirmation list should preserve the same distinction: ordinary OpenClaw review goes in a `可稍后处理`/deferrable group with low visual weight, while conflict and delete review groups remain visually risky. Row labels should say `不阻塞` or `可稍后` for ordinary review instead of using high-risk language.
...
If every remaining ordinary OpenClaw review item has been deferred, the first screen should become a completion/empty state: say `当前不用处理`, make `管理本机 skill` the primary action, and move `取消搁置` plus `查看确认清单` into secondary options. It should not make cancellation look like the required next step.
