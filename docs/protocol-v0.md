...
skill-sync approved-push --local-root /tmp/skill-sync-target --remote-snapshot ./cache-preview --last-applied-record /tmp/skill-sync-target/.skill-sync-backups/<apply-id>/.apply-record.json --blocked-report ./blocked-report/blocked-report.json --skill-id reviewed-skill --remote https://example.com/dav/cc-skill-sync --prefix snapshots/current --out ./approved-push --dry-run
skill-sync approved-push --local-root /tmp/skill-sync-target --remote-snapshot ./cache-preview --last-applied-record /tmp/skill-sync-target/.skill-sync-backups/<apply-id>/.apply-record.json --blocked-report ./blocked-report/blocked-report.json --skill-id reviewed-skill --remote https://example.com/dav/cc-skill-sync --prefix snapshots/current --out ./approved-push --yes
...
`approved-push` is the explicit approval path for local changes blocked by a writer policy, especially OpenClaw running as `pull-only`. It requires a `blocked-report.json`, one or more explicit `--skill-id` values, a current remote snapshot cache, and a remote destination. It refuses to run if the selected skill was not blocked by writer policy, is not currently a `push`/`push_new` candidate, or if its local/remote/base hashes changed since the blocked report was generated. Before upload, it also verifies that the live remote `index.json` still matches the local `--remote-snapshot` cache.

`approved-push` does not switch the daemon or unattended writer policy to `push-pull`. It builds a merged snapshot index from the cached remote snapshot plus only the approved local skill archives, uploads those approved archives and `index.json`, writes an approved-push audit record, and writes a new base record. Unselected local pushes stay local and continue to appear as blocked or pending changes in later reports.
