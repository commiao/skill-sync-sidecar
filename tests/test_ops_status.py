...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status["dashboard"]["operator"]["headline"], "OpenClaw 更新待确认")
            ...
            self.assertIn("保存到共享库", status["dashboard"]["operator"]["next_action"])
            ...
            self.assertIn('class="dashboard-sidebar"', DASHBOARD_HTML)
            self.assertIn('id="skills-hub-header"', DASHBOARD_HTML)
            self.assertIn('id="skills-sync-notice"', DASHBOARD_HTML)
            self.assertIn("function selectSkillsHubView", DASHBOARD_HTML)
            self.assertIn("function renderSkillsHubChrome", DASHBOARD_HTML)
            self.assertIn("class=\"dashboard-tabs\"", DASHBOARD_HTML)
            self.assertIn("data-dashboard-tab=\"workbench\"", DASHBOARD_HTML)
            ...
            self.assertIn("工作台", DASHBOARD_HTML)
            self.assertIn("id=\"simple-workbench-panel\"", DASHBOARD_HTML)
            self.assertIn("renderSimpleWorkbench", DASHBOARD_HTML)
            self.assertIn("openSimpleWorkbenchLibrary", DASHBOARD_HTML)
            self.assertIn("openSimpleWorkbenchPublish", DASHBOARD_HTML)
            ...
            self.assertNotIn("id=\"strip-scan-local\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"strip-dry-run\"", DASHBOARD_HTML)
            ...
            self.assertIn("不会自动写入共享库；确认后才会保存", DASHBOARD_HTML)
            ...
            self.assertNotIn("id=\"scope-scan\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"scope-dry-run\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"scope-publish\"", DASHBOARD_HTML)
            ...
            self.assertIn("两边都改过。现在只生成只读报告", DASHBOARD_HTML)
            ...
            self.assertIn("lastOperationFeedback", DASHBOARD_HTML)
            self.assertIn("simpleActionFeedbackHtml", DASHBOARD_HTML)
            self.assertIn("recentOperationFeedback", DASHBOARD_HTML)
            self.assertIn("shouldRememberOperationFeedback", DASHBOARD_HTML)
            self.assertIn("if (!cleanTitle || /^正在/.test(cleanTitle)) return false;", DASHBOARD_HTML)
            self.assertIn("kind === \"yellow\"", DASHBOARD_HTML)
            self.assertIn("kind === \"red\"", DASHBOARD_HTML)
            self.assertIn("没有写入|已拒绝|仍在修改|还有其他提醒|等待设备上报", DASHBOARD_HTML)
            self.assertIn("本次保存已完成", DASHBOARD_HTML)
            self.assertIn("操作边界：", DASHBOARD_HTML)
            self.assertIn("本页直接操作当前设备", DASHBOARD_HTML)
            self.assertIn("共享库只有确认保存才写入", DASHBOARD_HTML)
            self.assertIn("OpenClaw、Windows 和其他设备只展示状态", DASHBOARD_HTML)
            self.assertIn("simpleActionFactsHtml", DASHBOARD_HTML)
            self.assertIn("class=\"simple-action-facts\"", DASHBOARD_HTML)
            self.assertIn("状态概览", DASHBOARD_HTML)
            self.assertIn("simple-action-secondary", DASHBOARD_HTML)
            self.assertIn("<summary>其他选项</summary>", DASHBOARD_HTML)
            self.assertIn("simple-action-secondary-body", DASHBOARD_HTML)
            self.assertIn("simpleActionSecondaryHtml", DASHBOARD_HTML)
            self.assertIn("详情留在二级页面，需要时再看", DASHBOARD_HTML)
            self.assertIn("管理本机 skill", DASHBOARD_HTML)
            self.assertNotIn("renderSimpleActionMore", DASHBOARD_HTML)
            self.assertNotIn("simple-action-more", DASHBOARD_HTML)
            self.assertNotIn("查看详情（可选）", DASHBOARD_HTML)
            self.assertNotIn("simple-action-more-body", DASHBOARD_HTML)
            self.assertNotIn("查看清单<span>需要时再看原始队列", DASHBOARD_HTML)
            self.assertNotIn("看详情<span>只展开清单", DASHBOARD_HTML)
            self.assertIn("没有可保存更新", DASHBOARD_HTML)
            self.assertIn("不会自动删除", DASHBOARD_HTML)
            self.assertNotIn("renderSimpleDecisionList", DASHBOARD_HTML)
            self.assertNotIn("renderSimpleDecisionCard", DASHBOARD_HTML)
            ...
            self.assertIn("showLocalWriteGateHelp", DASHBOARD_HTML)
            self.assertIn("开启本机写入", DASHBOARD_HTML)
            self.assertIn("--allow-local-writes", DASHBOARD_HTML)
            ...
            self.assertIn("确认保存 OpenClaw 版", DASHBOARD_HTML)
            ...
            self.assertIn("先查看只读差异报告，再按推荐恢复、保存或手动处理", DASHBOARD_HTML)
            self.assertIn("approved=0", DASHBOARD_HTML)
            self.assertIn("没有写入共享库", DASHBOARD_HTML)
            self.assertIn("当前没有可保存更新。版本差异和删除确认不会通过这个按钮自动处理。", DASHBOARD_HTML)
            self.assertIn("本机助手未连接，无法执行检查或保存。", DASHBOARD_HTML)
            ...
            self.assertIn("手动导入不在列表里的 skill", DASHBOARD_HTML)
            ...
            self.assertIn("保存共享", DASHBOARD_HTML)
            self.assertIn("安装到本机工具", DASHBOARD_HTML)
            self.assertIn("检查共享", DASHBOARD_HTML)
            self.assertIn("保存到共享库", DASHBOARD_HTML)
            self.assertIn("updateLocalSkillGuide", DASHBOARD_HTML)
            self.assertIn("保存到共享库", DASHBOARD_HTML)
            self.assertNotIn("把已确认的更新同步出去", DASHBOARD_HTML)
            self.assertNotIn("id=\"easy-sync-empty\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"easy-sync-actions\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"easy-sync-steps\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"easy-dry-run\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"easy-publish\"", DASHBOARD_HTML)
            ...
            self.assertIn("publishRemainingFeedback", DASHBOARD_HTML)
            self.assertIn("保存完成，还有其他提醒", DASHBOARD_HTML)
            self.assertIn("保存完成，OpenClaw 又有新修改", DASHBOARD_HTML)
            self.assertIn("保存写入成功，等待设备上报", DASHBOARD_HTML)
            self.assertNotIn("发布已提交，等待状态收敛", DASHBOARD_HTML)
            ...
            self.assertIn("本次保存已完成", DASHBOARD_HTML)
            self.assertIn("这不是同一批保存失败", DASHBOARD_HTML)
            self.assertIn("相关确认项", DASHBOARD_HTML)
            self.assertIn("OpenClaw 仍在修改，已拒绝保存", DASHBOARD_HTML)
            ...
            self.assertIn("OpenClaw 修改可稍后处理", DASHBOARD_HTML)
            ...
            self.assertIn("reviewStatusKind", DASHBOARD_HTML)
            self.assertIn("reviewItemClass", DASHBOARD_HTML)
            self.assertIn("const ordinaryOnly = blocked > 0 && sourceChangedItems.length === blocked", DASHBOARD_HTML)
            self.assertIn("const pendingLabel = ordinaryOnly ? \"普通待审\" : \"待办\"", DASHBOARD_HTML)
            self.assertIn("OpenClaw 有新修改，可稍后处理。", DASHBOARD_HTML)
            self.assertIn("下一步：可稍后处理；继续管理本机 skill", DASHBOARD_HTML)
            self.assertIn("不阻塞", DASHBOARD_HTML)
            self.assertIn(".review-item.risky", DASHBOARD_HTML)
            self.assertIn(".review-group.risky", DASHBOARD_HTML)
            self.assertIn("继续管理本机 skill<span>新增、安装、保存共享。</span>", DASHBOARD_HTML)
            self.assertIn("这不是保存失败", DASHBOARD_HTML)
            ...
            self.assertNotIn("<button id=\"simple-defer-source\" type=\"button\" class=\"primary\" onclick=\"deferSourceChangedItems()\">先不提醒", DASHBOARD_HTML)
            ...
            self.assertIn("普通待审可稍后处理，不会阻塞本机工作区；OpenClaw 改完后，到二级确认清单检查最新版本。", DASHBOARD_HTML)
            self.assertIn("查看待审详情", DASHBOARD_HTML)
            self.assertNotIn("<button id=\"simple-dry-run\" type=\"button\" onclick=\"runExecutorAction('dry_run')\" disabled>检查最新版本</button>", DASHBOARD_HTML)
            ...
            self.assertIn("可以保存 OpenClaw 更新", DASHBOARD_HTML)
            self.assertIn("showPublishGateHelp", DASHBOARD_HTML)
            self.assertIn("开启保存权限", DASHBOARD_HTML)
            self.assertIn("本机助手当前只允许检查", DASHBOARD_HTML)
            ...
            self.assertIn("当前不用处理", DASHBOARD_HTML)
            ...
            self.assertIn("已搁置；需要重新处理时再取消搁置", DASHBOARD_HTML)
            self.assertIn("当前不用处理；需要重新检查或保存时再取消搁置", DASHBOARD_HTML)
            self.assertIn("当前没有需要你马上处理的同步事项", DASHBOARD_HTML)
            self.assertIn("需要重新处理时再取消搁置", DASHBOARD_HTML)
            self.assertIn("搁置不会写入任何位置；需要重新处理时再取消搁置", DASHBOARD_HTML)
            ...
            self.assertNotIn("id=\"local-workspace-refresh\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"local-workspace-dry-run\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"local-workspace-publish\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"executor-dry-run\"", DASHBOARD_HTML)
            self.assertNotIn("id=\"executor-publish\"", DASHBOARD_HTML)
            self.assertIn("粘贴路径后点“开始”", DASHBOARD_HTML)
            self.assertIn("管理本机 skill", DASHBOARD_HTML)
            self.assertIn("新增、安装、保存共享。", DASHBOARD_HTML)
            ...
            self.assertIn("id=\"local-skill-next\"", DASHBOARD_HTML)
            self.assertIn("local-skill-next", DASHBOARD_HTML)
            self.assertIn("查看保存明细", DASHBOARD_HTML)
            self.assertIn("setLocalSkillDetail", DASHBOARD_HTML)
            self.assertIn("setLocalSkillResult", DASHBOARD_HTML)
            self.assertIn("下一步：点“安装到本机工具”", DASHBOARD_HTML)
            self.assertIn("下一步：点“保存到共享库”", DASHBOARD_HTML)
            self.assertIn("需要跨设备复用时", DASHBOARD_HTML)
            ...
            self.assertIn("showLocalSkillPublishGateHelp", DASHBOARD_HTML)
            self.assertIn("showLocalWriteGateHelp", DASHBOARD_HTML)
            self.assertIn("开启本机写入", DASHBOARD_HTML)
            self.assertIn("查看说明；不会写本机。", DASHBOARD_HTML)
            self.assertIn("--allow-local-writes", DASHBOARD_HTML)
            self.assertIn("当前可以先检查共享；保存到共享库需要先开启保存权限", DASHBOARD_HTML)
            self.assertIn("先点“安装到本机工具”；要共享给其他设备", DASHBOARD_HTML)
            ...
            self.assertIn("id=\"skill-inventory-active-view\"", DASHBOARD_HTML)
            self.assertIn("skill-inventory-active-view", DASHBOARD_HTML)
            self.assertIn("renderSkillInventoryActiveView", DASHBOARD_HTML)
            self.assertIn("skillInventoryActiveViewModel", DASHBOARD_HTML)
            self.assertIn("skill-inventory-tool-summary", DASHBOARD_HTML)
            self.assertIn("aria-label=\"本机工具覆盖\"", DASHBOARD_HTML)
            self.assertIn("skillInventoryToolSummary", DASHBOARD_HTML)
            self.assertIn("已装：${escapeHtml(compactSkillList(installedLabels))}", DASHBOARD_HTML)
            self.assertIn("可装：${escapeHtml(compactSkillList(installableLabels))}", DASHBOARD_HTML)
            self.assertIn("id=\"skill-inventory-device-tool-overview\"", DASHBOARD_HTML)
            self.assertIn("renderSkillInventoryDeviceToolOverview", DASHBOARD_HTML)
            self.assertIn("openDeviceToolInventory", DASHBOARD_HTML)
            self.assertIn("本机未安装", DASHBOARD_HTML)
            self.assertIn("正在看：可安装到本机", DASHBOARD_HTML)
            self.assertIn("下一步：在每个 skill 行里勾选 Codex、Cursor 等工具", DASHBOARD_HTML)
            self.assertIn("正在看：本机已安装", DASHBOARD_HTML)
            self.assertIn("取消勾选会先检查并要求 REMOVE", DASHBOARD_HTML)
            self.assertIn("正在看：可保存共享", DASHBOARD_HTML)
            self.assertIn("先勾选要发布的 skill，再点“发布到中央仓库”", DASHBOARD_HTML)
            self.assertIn("正在看：项目级 skill", DASHBOARD_HTML)
            self.assertIn("正在看：全部 skill", DASHBOARD_HTML)
            ...
            self.assertIn('aria-label="搜索和筛选"', DASHBOARD_HTML)
            self.assertIn("已勾选项批量操作", DASHBOARD_HTML)
            self.assertIn("已勾选 ${escapeHtml(text(selectedItems.length))} 项", DASHBOARD_HTML)
            self.assertIn("先检查已勾选项，再一次确认发布到中央仓库", DASHBOARD_HTML)
            self.assertIn("skill-inventory-select", DASHBOARD_HTML)
            self.assertIn("toggleSkillInventorySelection", DASHBOARD_HTML)
            self.assertIn("selectedSkillInventoryItems", DASHBOARD_HTML)
            self.assertIn("clearSkillInventorySelection", DASHBOARD_HTML)
            self.assertIn("inventory-bulk-publish-button", DASHBOARD_HTML)
            self.assertIn("inventory-bulk-install-button", DASHBOARD_HTML)
            self.assertIn("inventory-bulk-remove-button", DASHBOARD_HTML)
            self.assertIn("publishSelectedSkillsToCentral", DASHBOARD_HTML)
            self.assertIn("bulkPublishCandidates", DASHBOARD_HTML)
            self.assertIn("installSelectedSkillsToTool", DASHBOARD_HTML)
            self.assertIn("uninstallSelectedSkillsFromTool", DASHBOARD_HTML)
            ...
            self.assertIn("bulkInstallCandidatesForTool", DASHBOARD_HTML)
            self.assertIn("bulkUninstallCandidatesForTool", DASHBOARD_HTML)
            self.assertIn("recentLocalToolChanges", DASHBOARD_HTML)
            self.assertIn("rememberLocalToolChanges", DASHBOARD_HTML)
            self.assertIn("recentLocalToolChangeFor", DASHBOARD_HTML)
            self.assertIn("recentSkillRowFeedback", DASHBOARD_HTML)
            self.assertIn("rememberSkillRowFeedback", DASHBOARD_HTML)
            self.assertIn("recentSkillRowFeedbackFor", DASHBOARD_HTML)
            self.assertIn("skillInventoryRowFeedbackHtml", DASHBOARD_HTML)
            self.assertIn("skill-inventory-row-feedback", DASHBOARD_HTML)
            self.assertIn("已安装到 ${toolLabel}", DASHBOARD_HTML)
            self.assertIn("安装已取消", DASHBOARD_HTML)
            self.assertIn("保存已取消", DASHBOARD_HTML)
            self.assertIn("保存共享库失败", DASHBOARD_HTML)
            self.assertIn("recent-installed", DASHBOARD_HTML)
            self.assertIn("recent-removed", DASHBOARD_HTML)
            self.assertIn("刚安装", DASHBOARD_HTML)
            ...
            self.assertIn("id=\"skill-inventory-current-client-tools\"", DASHBOARD_HTML)
            ...
            self.assertIn("currentClientId", DASHBOARD_HTML)
            ...
            self.assertIn("可保存共享", DASHBOARD_HTML)
            ...
            self.assertIn("未共享整理", DASHBOARD_HTML)
            self.assertIn("id=\"skill-inventory-guide\"", DASHBOARD_HTML)
            self.assertIn("Skill 清单推荐操作", DASHBOARD_HTML)
            self.assertIn("renderSkillInventoryGuide", DASHBOARD_HTML)
            self.assertIn("skillInventoryGuideModel", DASHBOARD_HTML)
            self.assertIn("pending_blocking", DASHBOARD_HTML)
            self.assertIn("pending_ordinary", DASHBOARD_HTML)
            self.assertIn("推荐先处理高风险同步", DASHBOARD_HTML)
            self.assertIn("普通待审，不阻塞本机", DASHBOARD_HTML)
            self.assertIn("OpenClaw 有普通待审", DASHBOARD_HTML)
            self.assertIn("这不阻塞本机工作区", DASHBOARD_HTML)
            self.assertIn("推荐先让本机工具可用", DASHBOARD_HTML)
            self.assertIn("查看可安装", DASHBOARD_HTML)
            self.assertIn("有本机 skill 可以共享", DASHBOARD_HTML)
            self.assertIn("查看可保存", DASHBOARD_HTML)
            self.assertIn("本机工具已经有可用 skill", DASHBOARD_HTML)
            self.assertIn("可保存公用", DASHBOARD_HTML)
            ...
            self.assertIn("skillInventoryNextActionText", DASHBOARD_HTML)
            self.assertIn("<strong>下一步</strong>", DASHBOARD_HTML)
            self.assertIn("勾选下面的本机工具；安装前会要求确认。", DASHBOARD_HTML)
            self.assertIn("点保存到共享库；会先检查，再要求 PUBLISH。", DASHBOARD_HTML)
            ...
            self.assertIn("refreshAfterLocalToolOperation", DASHBOARD_HTML)
            self.assertIn("refreshLocalWorkspace({ quiet: true })", DASHBOARD_HTML)
            self.assertIn("document.addEventListener(\"toggle\"", DASHBOARD_HTML)
            self.assertIn("openSkillInventoryGroups", DASHBOARD_HTML)
            self.assertIn("openSkillInventoryDetails", DASHBOARD_HTML)
            self.assertIn("skill-inventory-row-icon", DASHBOARD_HTML)
            self.assertIn("currentDashboardTab === \"skill-list\"", DASHBOARD_HTML)
            self.assertIn("openReviewDetailsForSkill", DASHBOARD_HTML)
            self.assertIn("去处理待确认", DASHBOARD_HTML)
            self.assertIn("data-review-skill-id", DASHBOARD_HTML)
            self.assertIn("review-item.focused", DASHBOARD_HTML)
            ...
            self.assertIn("把已勾选的 ${skillIds.length} 个共享库 skill 安装到 ${clientName} 的 ${toolLabel}", DASHBOARD_HTML)
            self.assertIn("把已勾选的 ${skillIds.length} 个 skill 从 ${clientName} 的 ${toolLabel} 可发现目录移走", DASHBOARD_HTML)
            self.assertIn("不会删除共享库内容", DASHBOARD_HTML)
            self.assertIn("保存到共享库", DASHBOARD_HTML)
            ...
            self.assertIn("设备私有不共享", DASHBOARD_HTML)
            ...
            self.assertIn("<button id=\"simple-defer-source\" type=\"button\" title=\"只隐藏首页提醒\" onclick=\"deferSourceChangedItems()\">先不提醒", DASHBOARD_HTML)
            self.assertNotIn("source-change-actions", DASHBOARD_HTML)
            self.assertNotIn("<button type=\"button\" onclick=\"deferSourceChangedItems()\">暂时搁置</button>", DASHBOARD_HTML)
            ...
            self.assertNotIn("<div class=\"workspace-actions\">", DASHBOARD_HTML)
            self.assertIn("<details class=\"workspace-secondary\">", DASHBOARD_HTML)
            ...
            self.assertIn("dashboardHealthLabel", DASHBOARD_HTML)
            self.assertIn("服务正常，有同步提醒", DASHBOARD_HTML)
            self.assertIn("服务正常", DASHBOARD_HTML)
            ...
            self.assertIn("有提醒", DASHBOARD_HTML)
    ...
    def test_dashboard_uses_live_blocked_items_when_report_is_stale(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("不是上次保存失败", status["dashboard"]["blocked_items"][0]["status_description"])
            self.assertEqual(status["dashboard"]["operator"]["headline"], "服务正常，OpenClaw 可稍后处理")
            self.assertEqual(status["dashboard"]["operator"]["next_action"], "可以继续管理本机 skill；OpenClaw 改完后再检查。")
            self.assertEqual(status["dashboard"]["operator"]["action_guide"]["title"], "OpenClaw 修改可稍后处理")
            self.assertIn("可以稍后处理", status["dashboard"]["operator"]["action_guide"]["summary"])
            self.assertIn("不应阻塞其他独立更新", status["dashboard"]["operator"]["action_guide"]["note"])
            devices = {device["id"]: device for device in status["dashboard"]["devices"]}
            self.assertEqual(devices["oc-vps"]["note"], "普通待审：OpenClaw 有新修改")
