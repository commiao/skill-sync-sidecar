...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("添加或同步 skill", DASHBOARD_HTML)
            self.assertIn("粘贴本机路径，安装到工具；需要共享时再保存到共享库。", DASHBOARD_HTML)
            ...
            self.assertIn("当前不用处理；普通待审已搁置，可以继续管理本机 skill", DASHBOARD_HTML)
            ...
            self.assertIn("3 保存共享库", DASHBOARD_HTML)
