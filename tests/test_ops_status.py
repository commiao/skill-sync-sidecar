...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("先看顶部结论；显示“同步已完成”就可以关闭。要新增或同步 skill，再展开可选操作。", DASHBOARD_HTML)
            ...
            self.assertIn("没有待办时不用点任何按钮", DASHBOARD_HTML)
            ...
            self.assertIn("刚刚发布完成，还有新的待办", DASHBOARD_HTML)
            ...
            self.assertIn("相关待处理项", DASHBOARD_HTML)
            ...
            self.assertIn("需要新增、安装或主动同步 skill", DASHBOARD_HTML)
