...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("同步已完成", DASHBOARD_HTML)
            self.assertIn("Mac、OpenClaw、共享仓库当前已对齐", DASHBOARD_HTML)
            self.assertIn("本页不会自动改其他设备；发布或跨设备写入前都会再问你", DASHBOARD_HTML)
            ...
            self.assertIn("先看差异", DASHBOARD_HTML)
            ...
            self.assertIn("先看 ${conflictItems.length} 个 skill 的差异", DASHBOARD_HTML)
            self.assertIn("两边都有改动", DASHBOARD_HTML)
            self.assertIn("生成只读报告", DASHBOARD_HTML)
            self.assertIn("不会发布、不会恢复、不会删除", DASHBOARD_HTML)
            self.assertIn("为什么停下来", DASHBOARD_HTML)
            ...
            self.assertIn("先检查 ${publishItems.length} 个待同步更新", DASHBOARD_HTML)
            self.assertIn("检查通过，可以同步 ${regularPublishItems.length} 个更新", DASHBOARD_HTML)
            self.assertIn("同步到中央库", DASHBOARD_HTML)
            ...
            self.assertIn("一键分析", DASHBOARD_HTML)
            self.assertIn("把已确认的更新同步出去", DASHBOARD_HTML)
            self.assertIn("同步到其他设备", DASHBOARD_HTML)
            self.assertIn("看到“同步已完成”才算完成", DASHBOARD_HTML)
            ...
            self.assertIn("可以继续同步到中央库", DASHBOARD_HTML)
            ...
            self.assertIn("先等 OpenClaw 改完", DASHBOARD_HTML)
            self.assertIn("不是按钮没反应", DASHBOARD_HTML)
            self.assertIn("只刷新状态；稳定后页面会重新给出检查/发布入口", DASHBOARD_HTML)
            ...
            self.assertIn("等待输入一个 skill 目录或 SKILL.md 路径。", DASHBOARD_HTML)
            self.assertIn("value=\"\" placeholder=\"粘贴 skill 目录或 SKILL.md 路径\"", DASHBOARD_HTML)
