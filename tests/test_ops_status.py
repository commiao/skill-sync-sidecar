...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("我确定 OpenClaw 上的是最新版</button>", DASHBOARD_HTML)
            self.assertIn("我确定中央仓库是正确版</button>", DASHBOARD_HTML)
            self.assertIn("我不确定，先看差异</button>", DASHBOARD_HTML)
            self.assertIn("会先预检，通过后输入 PUBLISH", DASHBOARD_HTML)
            self.assertIn("会先预检，通过后输入 RESTORE", DASHBOARD_HTML)
            self.assertIn("先看差异，只读查看，不会写任何地方", DASHBOARD_HTML)
