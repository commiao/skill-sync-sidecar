...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("本页直接操作当前 Mac", DASHBOARD_HTML)
            ...
            self.assertIn("OpenClaw 仍在更新", DASHBOARD_HTML)
            self.assertIn("OpenClaw 还在改", DASHBOARD_HTML)
            self.assertIn("下一步：还在改就点“先不提醒”；改完后再点“检查最新版本”。", DASHBOARD_HTML)
            ...
            self.assertIn("不用现在处理。改完后再检查并保存；想先做别的，点“先不提醒”。", DASHBOARD_HTML)
            self.assertIn("<button id=\"simple-defer-source\" type=\"button\" class=\"primary\" onclick=\"deferSourceChangedItems()\">先不提醒", DASHBOARD_HTML)
            self.assertIn("管理本机 skill<span>新增、安装、保存共享。</span>", DASHBOARD_HTML)
            self.assertIn("OpenClaw 还在改也不影响管理当前 Mac；检查按钮只读，不写共享库。", DASHBOARD_HTML)
            ...
            self.assertIn("当前客户端：Mac 本机", DASHBOARD_HTML)
            self.assertIn("这里的安装、移除和扫描只通过当前设备的本机助手执行。", DASHBOARD_HTML)
            self.assertIn("只安装到当前 Mac 的 Codex、Cursor、cc-switch 等本机工具。", DASHBOARD_HTML)
            ...
            self.assertIn("把当前筛选结果里的 ${skillIds.length} 个共享库 skill 安装到当前 Mac 的 ${toolLabel}", DASHBOARD_HTML)
            self.assertIn("把当前筛选结果里的 ${skillIds.length} 个 skill 从当前 Mac 的 ${toolLabel} 可发现目录移走", DASHBOARD_HTML)
    ...
    def test_dashboard_uses_live_blocked_items_when_report_is_stale(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status["dashboard"]["operator"]["headline"], "OpenClaw 仍在更新")
            self.assertEqual(status["dashboard"]["operator"]["next_action"], "还在改可以先放着；改完后点检查最新版本。")
