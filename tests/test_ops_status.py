...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("<button id=\"simple-defer-source\" type=\"button\" onclick=\"deferSourceChangedItems()\">先不提醒", DASHBOARD_HTML)
            self.assertIn("普通待审不会阻塞本机工作区；OpenClaw 改完后再检查最新版本。", DASHBOARD_HTML)
