...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("OpenClaw 新修改待审", DASHBOARD_HTML)
            self.assertIn("服务正常；这只是 OpenClaw 有新版本待确认，不影响你继续管理当前设备的 skill。", DASHBOARD_HTML)
            ...
            self.assertIn("普通待审不会阻塞本机工作区；OpenClaw 改完后，到二级确认清单检查最新版本。", DASHBOARD_HTML)
    ...
    def test_dashboard_uses_live_blocked_items_when_report_is_stale(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status["dashboard"]["operator"]["headline"], "服务正常，有 OpenClaw 待审")
            self.assertEqual(status["dashboard"]["operator"]["next_action"], "可以继续管理本机 skill；OpenClaw 改完后再检查最新版本。")
            self.assertEqual(status["dashboard"]["operator"]["action_guide"]["title"], "OpenClaw 新修改待审")
            self.assertIn("改完后再检查最新版本", status["dashboard"]["operator"]["action_guide"]["summary"])
