...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("不是待预检，需要选择保留 OpenClaw 版还是中央版", DASHBOARD_HTML)
            self.assertIn("直接选择保留哪边；不确定时再手动对比", DASHBOARD_HTML)
            ...
            self.assertIn("保留 OpenClaw 版</button>", DASHBOARD_HTML)
            self.assertIn("保留中央版</button>", DASHBOARD_HTML)
            self.assertIn("手动对比</button>", DASHBOARD_HTML)
