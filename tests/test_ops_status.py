...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("不处理同步也可以继续本机工作", DASHBOARD_HTML)
            ...
            self.assertIn("详情：为什么这样建议", DASHBOARD_HTML)
            ...
            self.assertIn("还在改可以先放着；改完后点“检查最新版本”", DASHBOARD_HTML)
            ...
            self.assertIn("<button type=\"button\" onclick=\"deferSourceChangedItems()\">先不提醒", DASHBOARD_HTML)
            self.assertNotIn("source-change-actions", DASHBOARD_HTML)
