...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("粘贴本机路径，安装到工具；需要共享时再发布到共享库。", DASHBOARD_HTML)
            ...
            self.assertIn("下一步：点“发布到共享库”", DASHBOARD_HTML)
            ...
            self.assertIn("正在看：可发布共享", DASHBOARD_HTML)
            ...
            self.assertIn("查看可发布", DASHBOARD_HTML)
