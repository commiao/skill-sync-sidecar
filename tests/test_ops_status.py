...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("本机优先", DASHBOARD_HTML)
            self.assertIn("本机可扫描和预检", DASHBOARD_HTML)
            ...
            self.assertIn("主操作区 · 只影响当前设备", DASHBOARD_HTML)
            ...
            self.assertIn("中央仓库", DASHBOARD_HTML)
            self.assertIn("设备地图", DASHBOARD_HTML)
