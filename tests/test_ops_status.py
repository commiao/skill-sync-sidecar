...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("预检审核", status["dashboard"]["operator"]["next_action"])
            ...
            self.assertIn("预检", status["dashboard"]["operator"]["top_issue"]["action"])
            ...
            self.assertIn("推送到中央仓库", status["dashboard"]["operator"]["next_action"])
            ...
            self.assertIn("预检", status["dashboard"]["blocked_items"][0]["operator_action"])
            self.assertEqual(
            ...
            self.assertIn("个版本差异", DASHBOARD_HTML)
            self.assertIn("不是待预检；先点“我不确定，先看差异”", DASHBOARD_HTML)
            ...
            self.assertIn("这里只展示当前要处理的一件事", DASHBOARD_HTML)
            ...
            self.assertIn("报告判断：OpenClaw 当前缺失，中央仓库仍有完整版本", DASHBOARD_HTML)
            ...
            self.assertIn("报告判断：中央仓库缺失，OpenClaw 仍有版本", DASHBOARD_HTML)
            ...
            self.assertIn("这会用中央仓库版本覆盖 OpenClaw", DASHBOARD_HTML)
            ...
            self.assertIn("本机执行器未连接，无法执行预检或发布。", DASHBOARD_HTML)
            ...
            self.assertIn("可选：查看三块状态（不影响当前操作）", DASHBOARD_HTML)
            ...
            self.assertIn("等待上方“确认发布”写入中央仓库", DASHBOARD_HTML)
            self.assertIn("重新预检", DASHBOARD_HTML)
            ...
            self.assertIn("可以继续确认发布到中央仓库", DASHBOARD_HTML)
            ...
            self.assertIn("查看预检命令", DASHBOARD_HTML)
            self.assertIn("确认缺失项是恢复还是删除", DASHBOARD_HTML)
            ...
            self.assertNotIn("冲突表示 OpenClaw 和中央仓库都改过", DASHBOARD_HTML)
            ...
            self.assertIn("可选：管理本机 skill / 查看技术细节", DASHBOARD_HTML)
            self.assertIn("workspace-overview", DASHBOARD_HTML)
            self.assertIn("只操作本机", DASHBOARD_HTML)
            self.assertIn("左边能操作当前 Mac；中央仓库和", DASHBOARD_HTML)
            self.assertLess(
                DASHBOARD_HTML.index("id=\"simple-action-panel\""),
                DASHBOARD_HTML.index("<section class=\"workspace-overview\""),
            )
            ...
            self.assertIn("只看会改什么，不写中央仓库", DASHBOARD_HTML)
            self.assertIn("确认无误后输入 PUBLISH", DASHBOARD_HTML)
            self.assertIn("1 扫描本机", DASHBOARD_HTML)
            self.assertIn("2 预检", DASHBOARD_HTML)
            self.assertIn("3 发布中央", DASHBOARD_HTML)
            ...
            self.assertIn("是唯一可直接操作的设备：先扫描，预检安全后", DASHBOARD_HTML)
