...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("会先检查当前筛选结果，全部安全后再要求 PUBLISH", DASHBOARD_HTML)
            ...
            self.assertIn("当前筛选结果批量操作", DASHBOARD_HTML)
            self.assertIn("批量处理当前筛选结果", DASHBOARD_HTML)
            ...
            self.assertIn("publishFilteredSkillsToCentral", DASHBOARD_HTML)
            self.assertIn("bulkPublishCandidates", DASHBOARD_HTML)
            self.assertIn("installFilteredSkillsToTool", DASHBOARD_HTML)
            ...
            self.assertIn("把当前筛选结果里的 ${skillIds.length} 个共享库 skill 安装到 ${clientName} 的 ${toolLabel}", DASHBOARD_HTML)
            self.assertIn("把当前筛选结果里的 ${skillIds.length} 个 skill 从 ${clientName} 的 ${toolLabel} 可发现目录移走", DASHBOARD_HTML)
