...
class OpsStatusTest(unittest.TestCase):
    ...
    def test_dashboard_status_reuses_ops_status_model(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("共享库只有确认发布才写入", DASHBOARD_HTML)
            ...
            self.assertIn("发布完成，还有其他提醒", DASHBOARD_HTML)
            self.assertIn("发布完成，OpenClaw 又有新修改", DASHBOARD_HTML)
            self.assertIn("发布写入成功，等待设备上报", DASHBOARD_HTML)
